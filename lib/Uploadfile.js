...
const Uploadfile = async (moduleParams, context) => {
  ...
  console.log("ImagePath", moduleParams.filepath); // console.log("Actual data:", request(moduleParams.filepath, function (error: any, response: any, body: any) {
  ...
  var download = function (uri, filename, callback) {
    request.head(uri, function (err, res, body) {
      ...
      console.log("OutputImage:", request(uri).pipe(fs.createWriteStream(filename)).on('close', callback)); // const imgpath=_writableState.path
    });
  };
  ...
  const folderId = JSON.stringify(moduleParams.Googledrivefolderid); // await saveFile(name, finalPath, imgtype, folderId).catch((error: any) => {
  //   console.error(error);
  // });
